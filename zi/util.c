...
void handleGaiStrErr(status_t status, errLevel_t errLvl, char* message, int error) {
   switch (errLvl) {
   case FATAL:
...
int Getnameinfo(const struct sockaddr* sa, socklen_t salen, char* host, socklen_t hostlen, char* serv,
   ...
   if (!message) message = "Getnameinfo fail";

   int res = getnameinfo(sa, salen, host, hostlen, serv, servlen, flags);
   if (res != 0) handleGaiStrErr(status, errLvl, message, res);
   return res;
}
...
int GetTCP_addrinfo(char* node, char* service, struct addrinfo** res, int flags, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "GetTCP_addrinfo fail";
...
int Socket(int domain, int type, int protocol, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "Socket fail";

   int sock = socket(domain, type, protocol);
   if (sock == -1) handleErr(status, errLvl, message);
   return sock;
}
...
int Accept(int fd, struct sockaddr* addr, socklen_t* restrict addr_len, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "Accept fail";
...
int Connect(int fd, const struct sockaddr* addr, socklen_t len, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "Connect fail";
...
size_t Writn(int fd, const void* vptr, size_t n, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "Writn fail";

   if (writn(fd, vptr, n) != n) handleErr(status, errLvl, message);
   return n;
}

ssize_t Recv(int socket, void* buffer, size_t length, int flags, status_t status, errLevel_t errLvl, char* message) {
   if (!message) message = "Recv fail";
...
int Select(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, struct timeval* timeout, status_t status,
           errLevel_t errLvl, char* message) {
   if (!message) message = "Select fail";
...
int bindTCP(char* port, status_t status) {
   ...
   GetTCP_addrinfo(NULL, port, &res, AI_PASSIVE, status, FATAL,
                   "getAddrInfo TCP for local address failed\n"); // na svim adrr

   int on  = 1; // SO_REUSEADDR
   int sfd = -1;
   for (p = res; p != NULL; p = p->ai_next) {
      sfd = Socket(res->ai_family, res->ai_socktype, res->ai_protocol, status, FATAL, "TCP socket creation failed\n");
      if (sfd < 0) { continue; }
      Setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(int), status, FATAL, "set SO_REUSEADDR failed");

      if (Bind(sfd, res->ai_addr, res->ai_addrlen, status, FATAL, "Unable to TCP bind\n") < 0) {
         close(sfd);
         continue;
      }
      break;
   ...
   if (p == NULL) { errx(status, "failed to bind TCP\n"); }
...
int bindUDP(char* port, status_t status) {
   struct addrinfo* res;
   GetUDP_addrinfo(NULL, port, &res, AI_PASSIVE, status, FATAL,
                   "getAddrInfo UDP for local address failed\n"); // na svim adrr
   int sfd = Socket(res->ai_family, res->ai_socktype, res->ai_protocol, status, FATAL, "UDP socket creation failed\n");

   int on = 1; // SO_REUSEADDR
   Setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(int), status, FATAL, "set SO_REUSEADDR failed");
   Bind(sfd, res->ai_addr, res->ai_addrlen, status, FATAL, "Unable to UDP bind\n");
