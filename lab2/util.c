...
const char *Inet_ntop(int af, const void *src, char *dst, socklen_t len, status_t status, errLevel_t errLvl,
                      char *message) {
    if (!message)
...
int GetUDP_addrinfo(char *node, char *service, struct addrinfo **res, int flags, status_t status,
                    errLevel_t errLvl, char *message) {
    if (!message)
...
int GetTCP_addrinfo(char *node, char *service, struct addrinfo **res, int flags, status_t status,
                    errLevel_t errLvl, char *message) {
    if (!message)
...
int Accept(int fd, struct sockaddr *addr, socklen_t *restrict addr_len, status_t status, errLevel_t errLvl,
           char *message) {
    if (!message)
        message = "Accept fail";
...
//            else
        ...
        if(delimiter != NULL && ptr[nread - 1] == *delimiter) { nleft +=1; break;}
...
ssize_t Send(int socket, const void *buffer, size_t length, int flags, status_t status, errLevel_t errLvl,
             char *message) {
    if (!message)
        message = "Send fail";
...
int Setsockopt(int fd, int level, int optname, const void *optval, socklen_t optlen, status_t status,
               errLevel_t errLvl, char *message) {
    if (!message)
